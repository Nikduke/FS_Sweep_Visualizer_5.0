...
- `preselection_payload` (object): server-precomputed selection-method candidate data
  - compact format (`format=compact_v6`) is the active payload contract:
    ...
    - `Peak |Z|` / `Peak X` exact modes use `peak_z_modes` / `peak_x_modes`; optional harmonic-band additions use `peak_z_band_modes` / `peak_x_band_modes`
    - `RX hull`, `Peak |Z|`, and `Peak X` use per-harmonic `Top N/h`, then union selected cases
...
14. Maintains source-aware selection model:
    ...
    - Peak |Z| / Peak X / Risk / Outliers method selections
